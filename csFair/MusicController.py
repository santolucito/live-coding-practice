...
class SampleListener(Leap.Listener):
    ...
    def on_frame(self, controller):
        ...
        max_dist_y = max_dist_y*(-1)+1

        if len(frame.hands) == 1:
            penta = [0,3,5,7,10]
            penta = (map(lambda y: map(lambda x:x+y,penta),[48,60,72]))
            penta = [item for sublist in penta for item in sublist]
            note = [penta[int(max_dist_x*len(penta))],max_dist_y]
            self.sendOSC("bigoh",note)
            print ("1 Hand Mode", note)

        if len(frame.hands) == 2:
            penta = [0,3,5,7,10]
            penta = (map(lambda y: map(lambda x:x+y,penta),[36,48]))
            penta = [item for sublist in penta for item in sublist]
            note = [penta[int(max_dist_x*len(penta))],max_dist_y]
            self.sendOSC("bigoh",note)
            print ("2 Hand Mode", note)

        if (frame.hands.is_empty):
            self.sendOSC("bigoh",[0,0])
